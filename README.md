# NOMP KawPoW Algorithm Pool -VirtualEmpire
Highly Efficient mining pool for Coins based on KawPoW algo!

This is opensource mining pool for VirtualEmpire, Please visit [VirtualEmpire](https://www.virtualempire.org) for more information

### Example site
[Virtualempireminer.eu](http://virtualempireminer.eu/stats/virtualempire)
...
| Project | Link |
| ------------- | ------------- |
| [KawPoWNOMP](https://github.com/ananthakannan92/kawpow-pool) | https://github.com/ananthakannan92/kawpow-pool |
...
### Install VirtualEmpire Daemon

    sudo adduser pool
    sudo usermod -aG sudo pool
    su - pool
    sudo apt install wget
    wget https://github.com/ananthakannan92/Virtualempire/releases/download/4.4.2.1/virtualempire-4.4.2.1.deb
    sudo dpkg -i virtualempire-4.4.2.1.deb
    mkdir -p ~/.virtualempire/
    touch ~/.virtualempire/virtualempire.conf
    echo "rpcuser=user1" > ~/.virtualempire/virtualempire.conf
    echo "rpcpassword=pass1" >> ~/.virtualempire/virtualempire.conf
    echo "prune=550" >> ~/.virtualempire/virtualempire.conf
    echo "daemon=1" >> ~/.virtualempire/virtualempire.conf
    virtualempired
    virtualempire-cli getnewaddress

Example output: HNRMPAYzdBHGWgK7CjSFByuUcufCHSST7r - it is the address of your pool, you need to remember it and specify it in the configuration file pool_configs/virtualempire.json.
    
    virtualempire-cli getaddressesbyaccount ""
    
Information about pool wallet address.
    
    virtualempire-cli getwalletinfo
    
Get more information.

    virtualempire-cli getblockcount
    
Information about synchronization of blocks in the main chain.

    virtualempire-cli help
...
### Install Pool
    ...
    git clone https://github.com/ananthakannan92/kawpow-pool
    cd kawpow-pool/
    ./install.sh
...
```javascript
{
    ...
    "defaultCoin": "virtualempire",
    
    "poollogo": "/static/icons/virtualempire.png",
    
    "discordtwitterfacebook": "",
    
    "pagetitle": "Virtual Empire Pool - 0% Fees Promo",
    "pageauthor": "ananthakannan",
    ...
    "pagekeywds": "GPU,CPU,Hash,Hashrate,Cryptocurrency,Crypto,Mining,Pool,Bitcoin,Virtualempire,Virtualempire,Wavi,Wavicoin,Dixicoin,Dixi,QBic,QBicCoin,Easy,Simple,How,To",
...
Create a pool config for you coins:
    ...
    mv pool_configs/virtualempire_example.json pool_configs/virtualempire.json

Change "address": "HQWTiHTAUVxUkrV72bqW5Tfrpgbzkde1Q4", to your pool created wallet address in file virtualempire.json:

    cd pool_configs
    nano virtualempire.json

```javascript
{
    "enabled": true,
    "coin": "virtualempire.json",
